...
const createPokemonCard = async pokemon => {
  ...
  if (pokemon.types.length === 2) {
    ...
    const stringHTML = `
		<div id="${pokemon.name}" class="poke-container" onClick="selectPokemon(${
      pokemon.id
    })">
			...
			<div id="${
        pokemon.name
      }_info" class="more_info" style="display:none; opacity: 0;">
    ...
    const stringHTML = `
		<div id="${
      pokemon.name
    }" class="poke-container" value="on" onClick="selectPokemon(${pokemon.id})">
			<div id="${pokemon.name}_container" class="poke-container">
				<div class="img-container">
					<img src="${pokemon.sprites["front_default"]}"/>
				</div>
